...
$.fn.cssReload = function (callback, newstyle, id) {
    var $this = $(this);
    if(newstyle){
     href = newstyle;  
    }else{
    var id = $this.attr("id"),
        href = $this.attr("href"),
        i = href.indexOf('?'),
        last_reload = 'reload=' + (new Date).getTime();
    if (i < 0) {
      href += '?' + last_reload;
    } else if (href.indexOf('reload=', i) < 0) {
      href += '&' + last_reload;
    } else {
      href = href.replace(/reload=\d+/, last_reload);
    }
    }
    yepnope.injectCss(href, function() {
         setTimeout(function(){
            if(callback)
               window[callback]();
...
$.fn.countdown = function (duration, message, callback) {
...
String.prototype.trunc = function (strLen, separator) {
