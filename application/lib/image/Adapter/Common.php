...
abstract class Common extends Adapter
{
    ...
    public function init()
    {
        ...
        if ($source instanceof \Nedo\Image\Source\File) {
            $this->loadFile($source->getFile(), $source->guessType());
        } else if ($source instanceof \Nedo\Image\Source\Create) {
            $this->createImage($source->getWidth(), $source->getHeight());
        } else if ($source instanceof \Nedo\Image\Source\Data) {
            $this->createImageFromData($source->getData());
        } else if ($source instanceof \Nedo\Image\Source\Resource) {
