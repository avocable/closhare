...
function execute_SQL() {
   ...
   $sql = str_replace(
           ...
           array($DB_NAME, $DB_PREFIX), $content);
...
function makeConfig(){
   ...
   $string = str_replace(
           ...
           array("1.0.0", $DB_HOST, $DB_USER, $DB_PASS, $DB_NAME, $DB_PREFIX, $ENC_KEY), $content);      
   }else{
      $r['result'] = 0;
   ...
   if(updateSiteInfo($ENC_KEY)){
      ...
      if($createConfigFile !== false){
         ...
         $encr = new \Nedo\Encryption($ENC_KEY);     
         ...
         if($renameSetupFolder != true){
            $r['result'] = 2;
            $r['msg'] = "I have <b>failed</b> while trying to rename the setup folder!<br>Please rename or delete it by hand.";
         }else{
            $_SESSION['d_first_em'] = $_POST['admin_mail'];
            $_SESSION['d_first_pa'] = $_POST['admin_pass'];
            return true;
         }
...
function updateSiteInfo($ENC_KEY){
   ...
   updateDB($DB_PREFIX.'settings', $settings, "id=".(int)1);
   
   //set admin
   $data = array(
       "user_name" => mysql_real_escape_string($_POST['admin_name']),
       "user_email" => mysql_real_escape_string($_POST['admin_mail']),
       "user_password" => $encr->encode(mysql_real_escape_string($_POST['admin_pass'])),
